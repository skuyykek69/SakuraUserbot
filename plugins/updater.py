...
UPSTREAM_REPO_URL = "https://github.com/levina-lab/veez_ultrobot"
...
async def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Sakura {ultroid_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Sakura {ultroid_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def upstream(ups):
    ...
    try:
        ...
        await eod(pagal, f"{txt}\n`directory {error} tidak ditemukan!`", time=10)
        repo.__del__()
        return
    ...
    if Var.HEROKU_API is not None:
        ...
        if not Var.HEROKU_APP_NAME:
            await eod(
                pagal,
                "`mohon atur `HEROKU_APP_NAME` variable agar dapat memperbarui userbot.`",
                time=10,
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await eod(
                pagal,
                f"{txt}\n`credentials heroku tidak valid untuk memperbarui dyno userbot.`",
                time=10,
            )
            repo.__del__()
            return
        ...
        try:
            ...
            await eod(pagal, f"{txt}\n`ini dia log yang error:\n{error}`", time=10)
            repo.__del__()
            return
        await eod(pagal, "`berhasil diperbarui!\nmemulai ulang, tunggu sebentar...`", time=60)
    else:
